...
InkShader is a Web-based font editor that uses Paper.js for Bezier curve rendering. The frontend is a static ES-module application with no build step (`index.html`, `css/`, `js/`, `assets/` at the repository root). An optional local backend (`backend/`, FastAPI) provides fonttools-based font operations (remove overlap, direction correction, OTF/TTF export) and the desktop shell; without it the frontend runs standalone and the corresponding menu items are disabled.
...
| `js/vendor/` | Vendored third-party builds and their ESM adapters (see `js/vendor/README.md`) |
| `assets/icons/` | SVG tool icons |
| `js/schemas/project_schema.json` | File save format spec (authoritative, referenced by S014) |
| `backend/` | Local backend: FastAPI service (`src/inkshader/api/`), fonttools operations (`src/inkshader/font_ops.py`), desktop shell (`src/inkshader/desktop/` — desktop mode only; the web build excludes pywebview/pythonnet), packaging (`build.py`, `packaging/web.spec`); dependencies and tool config in `pyproject.toml` (single source) |
| `backend/tests/` | pytest unit tests for the backend (`./dev.ps1 test`) |
...
| File | Responsibility |
|------|---------------|
| `services/i18n.js` | Internationalization (`I18nManager.t()`); the `en` table is the single source of all user-visible text |
...
| G001–G011 | Coding standards | [CODEGUIDE.md](CODEGUIDE.md) |
...
`npm run check:i18n` fails on a key that is referenced but undefined, or defined
