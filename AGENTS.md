...
**Objective**: User request: remove the Font/Kerning/Glyphs items from the top menu bar and convert those three popups (`font-popup`, `kern-popup`, `glyph-popup`) into DOCK PANELS, with Show/Hide toggles in the Edit menu whose checkmark state stays in sync with panel visibility.

**Changes**:
1. **`index.html`** — Removed the three `.item` entries (Font/Kerning/Glyphs); the menu bar now has File/Edit/Preferences/Help only.
2. **`js/services/i18n.js`** — Keys renamed (en + zh): `menu.font/kerning/glyphs` → `panel.font` ("Font"/"字体"), `panel.kerning` ("Kerning"/"字偶距"), `panel.glyphs` ("Glyphs"/"字形"). `I18nManager.init()` already installs a MutationObserver on `document.body` (childList+subtree, re-runs `translateDOM()` when a node with `[data-i18n]` is added), so the title bars created by connectedCallback are auto-translated — no extra wiring needed.
3. **`js/ui/dock_layout.js`** — `PANEL_DEFS` += font/kerning/glyphs (compSelector `font-popup`/`kern-popup`/`glyph-popup`); constructor gains `this._hiddenPanels = new Set()`; `initialize(panelIds)` NEVER auto-adds the three optional ids (restored layout without a leaf → add to hidden set; fresh layout → seed all three hidden; tree built from visibleIds); storage blob gains `hiddenPanels: Array.from(...)` (save + restore). New public API: `isPanelHidden(panelId)`, `showPanel(panelId)` (delete hidden flag + `comp.dataset.panelHidden`, add leaf if absent, `_buildDOM` + `_activateTab`, short-circuit if floated), `hidePanel(panelId)` (set flag, `_removePanelFromFloat` if floated, `_removeLeaf` + `_rebuild` if leaf exists). Both guard `this._restoring`.
4. **`js/ui/font_popup.js`** — Title bar `<div class="prop_panel_title_wrapper"><span class="panel_title" data-i18n="panel.font">Font</span></div>`; `_visible`, `show()`, `hide()` and the document-mousedown outside-click handler removed; connectedCallback guarded by `_domReady` (re-attach safe); STATE_CHANGED listener checks `this.dataset.panelHidden` before `_loadSettings()`; `setProjectManager`/`setCanvas` retained but no longer called by the shell (components fall back to `document.querySelector('main-canvas')`).
5. **`js/ui/kern_popup.js`** — Title bar (panel.kerning); `_visible`/`show()`/`hide()`/`_awayHandler`/`disconnectedCallback` teardown removed; `appEventBus.on(STATE_CHANGED)` registered ONCE (no teardown — disconnect/reattach would otherwise kill the refresh), guarded by `dataset.panelHidden`; initial `_populateGlyphSelects()` + `_renderPairs()` moved into connectedCallback; add/remove/edit already route through `CanvasDispatcher.requestSetKerningPairs` (Session 20).
6. **`js/ui/glyph_popup.js`** — Full inline rewrite: connectedCallback builds a title bar (panel.glyphs) + a persistent `.sequence-add-menu` inside the component (old code appended a floating menu to `<body>` and removed it per-show). `show()`/`hide()`/`disconnectedCallback`/positioning gone; `_buildMenu()` builds header/form/char-grid/other-groups once; the STATE_CHANGED listener is registered once with a `dataset.panelHidden` guard (`TREE_REVISION` → rebuild maps + `refreshSections`); grid columns fixed at 8 (`static get COLS`). All item/rename/delete handlers identical to the old popup minus `this.hide()` calls.
7. **`js/ui/layout_shell.js`** — The three components are now mounted BEFORE `DockLayout.initialize()` (initialize captures their elements into `_componentRefs` — show/hide moves those exact elements); panelIds extended to `["canvas","objects","properties","console","sample","font","kerning","glyphs"]`; remounted AFTER the early-return guard. Removed: `btnFont`/`btnKern`/`btnGlyphs` finders, their click handlers, the three `popup.hide` monkey-patches, and the font/kern/glyph branches of `closeAnyOpenMenu()`. Edit menu: added `makePanelToggle(i18nKey, panelId)` — reads `dock.isPanelHidden()` at menu-BUILD time (items rebuilt on every click → checkmark always fresh), label = `'✓ ' | '   '` + `I18nManager.t(key)`; action toggles `dock.showPanel/hidePanel`. Three items placed after the snap toggles (`panel.font/kerning/glyphs` labels show as "Font"/"Kerning"/"Glyphs").
8. **`css/style.css`** — `.dock-content >` borderless list += `font-popup`, `kern-popup`, `glyph-popup`. New dock-context override block: `.dock-content > font-popup/kern-popup/glyph-popup` = `position: static; display: flex; flex-direction: column;` + transparent background / no blur / no border / no shadow / `flex: 1; min-height: 0` (kills the popup chrome `position: fixed; display: none` from the popup base block); the panels' `.pen-tool-popup-body` (font/kern) and `.sequence-add-menu` (glyph — which carries its OWN `position: fixed` in the Menus section) flex to fill and scroll. Title bars are auto-hidden in dock leaves by the existing `.dock-tabs .dock-leaf .prop_panel_title_wrapper { display: none }` convention (tab bar provides the label; drag works via tab or via the title once floated).

**Verification** (pure-Node CDP driver `test/dock_panels_driver.mjs`, headless Chrome :9222, server :8123, CDP Network.clearBrowserCache before every navigation, `window.confirm = () => true` + error collector installed via Page.addScriptToEvaluateOnNewDocument, localStorage cleared + reload for a clean start — **30/30 checks ALL GREEN**):
- Menu bar = File/Edit/Preferences/Help only; `#menu_kerning`/`#menu_glyphs` gone; all three panels hidden by default (no dock leaves); components mounted in body outside the dock.
- Title bars carry `data-i18n` panel.*; glyph grid renders 95 cells with A mapped to char 65; kern selects populated (12 options after baseline load); kern pair rows render (A→E −500); font popup fields load the project (UPM 1000).
- Edit menu: three unchecked toggles; clicking Font shows the panel (leaf + component inside `.dock-content`, stored hiddenPanels = [kerning,glyphs], computed position static/display flex); reopening shows `✓ Font` while Kerning stays unchecked; toggling off removes the leaf and re-adds font to hiddenPanels.
- `dataset.panelHidden` lifecycle: set to "1" on hide AND the component becomes DETACHED from the document (held only in `__dock._componentRefs` — probe must not querySelector it while hidden); removed + reattached on show.
- i18n: title bars translate 字体/字偶距/字形 under zh and revert under en (MutationObserver path verified).
- Persistence: visible panels survive reload (stored hiddenPanels []); hidden panels survive reload; an OLD v2 layout blob without hiddenPanels and without the three ids migrates to all-hidden (initialize auto-adds them to the hidden set).
- Zero app errors across the whole session (error collector).
- LSP diagnostics clean: index.html/i18n.js/dock_layout.js/font_popup.js/kern_popup.js/glyph_popup.js/layout_shell.js/style.css.

**Notes for future sessions**:
- **Hidden-panel detachment contract**: hiding an optional panel REMOVES its element from the document entirely (it survives only in `DockLayout._componentRefs`; state preserved via the custom element). Probes must reach hidden components through `window.__dock._componentRefs[id]`, never `document.querySelector`. Re-showing re-attaches the SAME element via `_attachComponentElements`.
- **dataset.panelHidden contract**: `hidePanel` sets it (components skip STATE_CHANGED work while detached); `showPanel` deletes it. It is NOT persisted — after a reload, hidden components are detached but lack the flag, so their STATE_CHANGED handlers may do wasted (harmless) work until the next hide/show cycle. Acceptable; do not "fix" by re-marking on restore unless a real defect appears.
- **Check-state freshness**: the Edit menu items are rebuilt on every open, so `makePanelToggle` reads `dock.isPanelHidden()` at build time — never cache the checked state outside the menu build.
- **Dropdown item click**: `.save-dropdown-item` has a `click` listener that calls `this.hide()` then the action — programmatic `.click()` on the item element works; submenus are appended to `<body>`, so query item elements only within `dropdown-menu` (or you'll match submenu copies).
- **i18n MutationObserver**: any component that adds `[data-i18n]` DOM after load relies on `I18nManager.init()`'s MutationObserver (already active — translateDOM runs on subtree additions). No manual translate calls needed.
- **Optional-panel registration order**: the three components must exist in the DOM BEFORE `dock.initialize()` (or `_componentRefs` misses them and showPanel finds no element). The shell mounts them right after the guard, before `new DockLayout(...)`.
- Canonical probe: `test/dock_panels_driver.mjs` (30 checks, **44 as of Session 24** — rerun for any dock / menu-bar / i18n / these-three-panels change). test/ inventory now 50 files (35 probes/drivers + 15 fixtures/inputs).

### Session 24 (current)

**Objective**: User report: "组件并列过多导致拖动系统异常" — with many components side-by-side the drag system misbehaved. Three-part fix: ① **resize clamp** — dragging splits must keep every split's percentage sum EXACTLY 100 at all times (no drift); ② **min-size-aware capacity** — panels have hard pixel minimums (200px wide / 100px high); when a drop would overflow the container's fitted slots the new panel folds into the nearest tabs (auto-tabs) instead of forcing more columns; ③ **Edit menu** — Show/Hide toggles for ALL 8 panels (canvas/objects/properties/console/sample/font/kerning/glyphs), checkmark synced with visibility, persisted in the layout blob (extends Session 23's trio to every panel, floated panels included).

**D7/D8 drift root cause**: the resize clamp math was ALREADY exact — `_onResizeMove` (L1208-1255) computes take/return pairs losslessly (each branch verified take ≡ add modulo fp-ulp; no toFixed/10000/Math.round anywhere in dock_layout.js, grep-verified). The drift entered through the BASELINE: `_startResize` read starting percentages from the DOM (`parsePct` on `el.style.flex`, ~L1190). Chrome's CSSOM re-serializes inline flex-basis percentages to ~6 significant digits — `flex: 1 1 33.333333333333336%` reads back as `"33.3333%"`. Every drag started from a truncated world ([33.3333 ×3], sum 99.9999) and the exact clamp faithfully PRESERVED that sum into the tree + storage. Pre-fix D7 sizes [27.247956403269757, 39.41864359673025, 33.3333], sum 99.9999, where 39.41864359673025 = 33.3333 + (33.3333 − 27.247956403269757) EXACTLY — proof the math was exact and the DOM baseline was the corruptor.

**Changes**:
1. **`js/ui/dock_layout.js` `_startResize`** — `startPcts` now sourced from the full-precision split tree (`splitEl._treeNode.sizes`, guarded: same length + all finite numbers), falling back to DOM `parsePct` only when the tree data is unavailable. Post-fix D7: sizes [27.247956403269757, 39.41871026339692, 33.333333333333336], sum EXACTLY 100 (minPct = max(5, 100/367×100) = 27.247956403269757 — smallest panel clamped to its 100px minimum, sibling takes the remainder, untouched panel keeps its exact 33.333333333333336).
2. **`js/ui/dock_layout.js`** — constants L1-26: `MIN_PANEL_WIDTH=200`, `MIN_PANEL_HEIGHT=100`, `RESIZER_GAP=6`. L57-114 `initialize` with all 8 panelIds + generalized `_hiddenPanels` seeding (fresh layout hides the trio). L130-308 capacity machinery: `_maxSlots(extent, min) = floor((extent + gap) / (min + gap))` (gap counted per slot — a 367px container fits exactly 3 v-slots), `_containerExtent`, `_nodeExtentPx`, `_fitsNewSlot` (`maxSlots >= children.length + 1`; an empty root split always fits), `_addNodeToTree` (preferred-axis `_addNodeToSplit`, else perpendicular wrap), `_addNodeToSplit` (fit → push + renormalize 100/n; overflow → descend into a split child or absorb into the LAST leaf/tabs child), `_absorbIntoTabs`, `_normalizeOverflowingSplits` (post-restore guard: folds over-capacity splits into tabs, re-serializes the normalized tree to storage). `showPanel`/`hidePanel` extended to ALL panels (hide of a floated panel runs `_removePanelFromFloat`; leaf removal via `_removeLeaf` L1722 splice + renormalize + `_rebuild`; both guard `this._restoring`).
3. **`js/ui/layout_shell.js`** — Edit menu: 8 `makePanelToggle(i18nKey, panelId)` items (five core panels + the Session 23 trio), checkmark read fresh at menu build via `dock.isPanelHidden()`.
4. **`js/services/i18n.js`** — new `panel.canvas/objects/properties/console/sample` keys en+zh (画布/对象/属性/控制台/样张; font/kerning/glyphs keys exist since Session 23).
5. **`test/dock_panels_driver.mjs`** — extended from 30 to **44 checks** (Phase A 6 + Phase B 19 + Phase C 5 + Phase D 14): capacity/fit/overflow-tabs/aggressive-drag drift/storage checks.

**Verification** (pure-Node CDP driver, headless Chrome :9222, :8123 — **44/44 ALL GREEN**):
- A/B/C regressions intact: menu bar, toggles, hidden defaults, reload persistence, old-layout migration, zero app errors.
- D1/D2: 8 toggles; zh labels for the five core panels. D3/D4: console hide/show (component detached, ref kept). D5/D6: canvas (CORE panel) hide/show — detaches alive, reattaches and re-renders.
- D7/D8: aggressive drag → sum exactly 100 AND persisted sum 100 (pre-fix: 99.9999).
- D9: 800px fresh layout fits capacity (2 columns, trio hidden). D10: drop into a full split → auto-tabs(canvas,font). D11: perpendicular drop with room → new v-split wrap. D12: crowded saved layout restores with overflow folded into a 4-tab, normalized tree persisted. D13: deserialize drops hidden-panel leaves. D14: zero app errors through Phase D.
- LSP diagnostics clean: dock_layout.js + all changed files.

**Notes for future sessions**:
- **NEVER derive drag baselines from DOM style readback.** Chrome re-serializes inline percentages with ~6 significant digits (`33.333333333333336%` → `"33.3333%"`), so DOM-parsed "%" values are 4-decimal truncated — a 100/3 property layout reads back as exactly 33.3333×3, summing to 99.9999. The tree `sizes` array is full-precision; it is the ONLY valid source for `_startResize` baselines.
- **Clamp math contract**: `_onResizeMove` take/return arithmetic is lossless; drift can ONLY re-enter through baselines or renormalization. Keep tree-sourced startPcts; never round anything on the resize path.
- **Capacity contract**: `floor((extent + gap)/(min + gap))` counts the resizer gap per slot; `_fitsNewSlot` uses `maxSlots >= children + 1`; overflow folds into the NEAREST tabs via last-child absorption; perpendicular drops wrap into a NEW nested split only when the preferred axis is full but the perpendicular direction has room (D10 vs D11).
- **Hidden-panel generality** (extends Session 23): show/hide work for ALL panels incl. float removal; Edit-menu checkmarks rebuild per open.
- Driver gotcha: the trusted D7/D8 numbers are the POST-fix ones (sum exactly 100, untouched panel stays 33.333333333333336) — do not "restore" the pre-fix 99.9999 expectations.
- Canonical probe: `test/dock_panels_driver.mjs` (44 checks — rerun for any dock/resize/capacity/i18n/menu change).

### Session 25 (current)

**Objective**: User report: "我强调过隐藏组件需要处理同步问题，但目前毫无处理，隐藏画布立刻引发大量问题" — hidden panels (canvas in particular) had NO sync handling. Evidence gathered first: hiding the canvas broke (S3b) `CanvasDispatcher.requestSetFontSettings` (stack stayed 0, upm untouched), broke (S3c) undo, and on re-show (S4) the canvas revived as a collapsed 1px strip (734×32, fixture offset {158,-193} obliterated, ink=0).

**Root causes**:
1. **Disconnect killed the window event-bus listeners**: canvas's `onBus` cleanups were pushed into `globalEventTrackers`, and `disconnectedCallback` clears ALL of it on disconnect. The window-bus listeners are PERMANENT (registered once, module scope) — tearing them down on hide dropped every dispatch (request-set-font-settings arrives during the hidden window) and state sync went dead until reconnect re-registration.
2. **hidePanel kept no slot memory**: the leaf subtree was removed with no record of where it lived; `showPanel` re-added the panel from scratch (new leaf, no tree slot, defaults) → 1px strip, lost geometry/offset; also the OLD code returned early when the panel existed in `_hiddenPanels` list but had no tree slot.

**Changes**:
1. **`js/presentation/canvas/canvas_controller.js`** (onBus ~L40-43) — cleanups now push into `this.canvas.globalBusTrackers` (NEVER cleaned on disconnect). `reconnect()` is now an intentional empty no-op (signature kept; re-registration would duplicate listeners — window-bus listeners are permanently alive, DOM listeners re-bind via `canvasInputController.bind()` gen>1 path). **`js/canvas/main_canvas.js`** — constructor adds `this.globalBusTrackers = []`; `disconnectedCallback` now cleans ONLY `globalEventTrackers` (DOM-side: pointer/keyboard/etc.), with a Session 25 comment explaining the split.
2. **`js/ui/dock_layout.js`** — slot-memory system: constructor `_hiddenSlots = {}`; new methods `_findLeafSlot(panelId)` (search under a root: frame-root / tabs root / split root), `_captureHiddenSlot(panelId, slot)` (mode root / tabs {tabsRef,index} / sibling {orientation,index,size,siblingRef} / split {parentRef,index,sizes}), `_isNodeReachable(node)` (still in the live tree), `_replaceNode(oldNode, newNode, deleteSizes)`, `_restoreHiddenSlot(panelId, replacement)` (rebuilds the slot branch, deletes the memory entry), `_serializableHiddenSlots()` (only root / sibling-where-sibling-is-root — the two modes restorable across reload). `hidePanel` captures the slot BEFORE `_removeLeaf`. `showPanel` first tries `_restoreHiddenSlot(panelId, leaf)` and falls back to `_addPanelLeaf` (default branch) — the memory is CONSUMED and deleted only after the restore attempt (self-caught bug in run1: the delete ran BEFORE the restore read it, and S4 still collapsed to 32px). Storage save (`_saveStateToStorage`) includes `hiddenSlots: this._serializableHiddenSlots()`; restore feeds valid modes back into `_hiddenSlots` at boot. Floated-panel reveal (Edit menu) discards the memory.

**Verification** (pure-Node CDP probe `test/canvas_hide_sync_probe.mjs` — now with HARD assertions, 5/5 PASS; headless Chrome :9222, server :8123, Network.clearBrowserCache, baseline `test/InkShader_project_2026-08-02T16-27-25.json`; + `test/dock_panels_driver.mjs` 44/44 ALL GREEN — no dock regression):
- S3b dispatch WHILE HIDDEN (real window-CustomEvent path): stack 0→1, upm 2000, dirty, loop stopped (PASS — pre-fix: stack stayed 0, upm 1000).
- S3c undo WHILE HIDDEN: stack 1→0, upm 1000 (PASS — pre-fix: undo did nothing).
- S4 re-show: rect [28,53,543,340] EXACTLY (pre-fix 734×32), backing 543×340, offset {62.5,-39} restored, tree back to h-split [75,25] with canvas as left leaf, loop running, no canvas-in-tabs (PASS).
- S4b/S4c post-re-show dispatch+undo: stack 0→1→0, upm 1500→1000 (PASS).
- S5 hide → RELOAD → show: canvas restored 543×340 with the PERSISTED slot (hiddenSlots in blob), render ink 1088, FINAL errs [] (PASS — proves cross-reload persistence works).
- `PROBE run3`: 5/5 assertions PASS, `SUMMARY: all session-25 checks passed`, exit 0. LSP diagnostics clean: canvas_controller.js, main_canvas.js, dock_layout.js.
- Driver gotcha worked fine: dock driver unaffected by the slot-memory changes (it hides into its own managed slots).

**Notes for future sessions**:
- **Two-track tracker invariant**: `globalEventTrackers` = DOM-side listeners (must re-bind on reattach — cleaned by disconnectedCallback); `globalBusTrackers` = window event-bus listeners (PERMANENT, never cleaned). Any new persistent listener MUST go to globalBusTrackers or hidden-state sync dies again. Never "clean up" the bus trackers on disconnect; `reconnect()` stays a no-op.
- **Slot-memory modes**: only `root` and `sibling`-with-root survive serialization (they restore into a known root). `tabs`/`split` modes are runtime-only memory (their parents are too deep to rebuild reliably across reload) — a hidden panel restored from `tabs`/`split` memory naturally falls back to `_addPanelLeaf` on the next boot, which is the same behavior as Session 23/24 already-verified appends.
- **Restore-then-delete ordering**: `showPanel` must call `_restoreHiddenSlot` BEFORE deleting the memory entry (the delete-before-read bug collapsed S4 in run1).
- Canonical probe: `test/canvas_hide_sync_probe.mjs` (S1b/S3b/S3c/S4/S4b/S4c/S5 stages + 5 hard assertions — rerun for ANY canvas lifecycle / dock hide-show / dispatcher change; assertions encode the Session 25 invariants).

### Session 26 (current)

**Objective**: User vetoed the destroy-based hide + slot-memory design of Session 25: "canvas隐藏时一切都不正常。隐藏组件只应该是一个ui动作，我觉得这是一件很简单的事，如果你直接把他销毁了当然会有问题". New invariant: **hiding a panel is a PURE UI action** — the leaf ALWAYS stays in the dock tree and the DOM, the component is NEVER disconnected/destroyed, geometry is preserved by leaving split sizes untouched (no slot memory needed). Session 25's whole slot-memory system (`_hiddenSlots`/`_captureHiddenSlot`/`_restoreHiddenSlot`/`_serializableHiddenSlots`/`_findLeafSlot`/`_replaceNode`/`_isNodeReachable`) was REMOVED.

**Changes** (all `js/ui/dock_layout.js` unless noted):
1. **Pure-CSS hide/show**: `hidePanel` = `_hiddenPanels.add` + `leaf.classList.add("dock-hidden")` + `dataset.panelHidden`; `showPanel` = delete flag + remove class (`leaf` STILL in tree — never `_removeLeaf`). `.dock-hidden { display: none !important; }` at `css/style.css` L705-706 (sizes/flex-basis untouched → re-show restores exact geometry). Only a panel that was NEVER in the tree (e.g. font at boot, Session 23 default-hidden) still goes through `_addPanelLeaf` on show.
2. **`_applyHiddenStateToDOM()`** (L1838) — after every DOM rebuild (deserialize L336, `_buildDOM` L451) re-applies `.dock-hidden` to all `_hiddenPanels` leaves; when the hidden leaf holds the group's activeIndex and the group has ≥2 leaves, hands off the active tab (see 3). A leaf that was CSS-hidden via an inactive tab needs no hand-off.
3. **`_handOffHiddenTab(tabsEl, hiddenLeafIdx)`** (L1819-1828) — shared helper: scans `(idx + step) % n` for the FIRST leaf NOT in `_hiddenPanels` and `_activateTab`s it; if every leaf is hidden, returns (all stay hidden). Used by BOTH `_applyHiddenStateToDOM` and `hidePanel`'s active-tab branch (previously two divergent inline implementations).
4. **`hidePanel` float branch** — floated panels now hide the float window (not destroy it); the float restore path in `_restoreFromStorage` (L402-427) re-applies `.dock-hidden` to float elements whose panelId is in `_hiddenPanels` (fixes: hidden floated panel reappearing after reload).
5. **driver assertions re-semanticized** (`test/dock_panels_driver.mjs`): B8/B19/C3/D3/D5/D13 now assert leaf retention + `.dock-hidden` + component attached; D13 flipped from "deserialize drops hidden-panel leaves" (Session 24 wording) to "keeps hidden-panel leaves (CSS-hidden)".

**Verification**:
- `test/dock_panels_driver.mjs` 44/44 ALL GREEN again (B8: font leaf stays + cssHidden + attached; B19: panelHidden flag set/removed, component NEVER detached; C3: hidden panels persist through reload as leaves; D3/D5: console/canvas hidden with leaf + component attached; D13: deserialize keeps hidden leaves).
- `test/canvas_hide_sync_probe.mjs` 7/7 PASS: S2 hide keeps `backing [543,340]`, loop running, sync alive (ZERO disconnect); S3b/S3c dispatch+undo WHILE HIDDEN work (bus listeners permanent — Session 25's globalBusTrackers split holds); S4 re-show restores rect [28,53,543,340] EXACTLY + tree [75,25]; S5 reload → hidden canvas leaf persists IN THE TREE (75% slot), re-show → 543×340, 0 errs.
- **S5 probe-ordering lesson** (root cause of the only failing run): the original probe showed `font` BEFORE re-showing canvas — `showPanel('font')` has no tree leaf, so `_addPanelLeaf` re-splits the root from [75,25] to [33,33,33] (762px container legitimately fits 3×200px slots per Session 24 capacity) and the canvas lands at ~33% (backing ~225). That is the CAPACITY CONTRACT working as designed, NOT a hide regression. Fixed by reordering the probe: verify the canvas re-show restore FIRST (720 lines of assertions untouched), then interact with font. S5 check renamed "(persisted tree slot)".
- LSP diagnostics clean: dock_layout.js (only file changed in the session's app code). Test probe debug output confirmed via `test/canvas_hide_sync_probe.mjs` run: `SUMMARY: all canvas hide/sync checks passed`, exit 0, FINAL errs [].

**Notes for future sessions**:
- **Session 26 hiding contract (SUPERSEDES Session 25)**: hide = CSS class on the leaf; leaf + component stay in tree/DOM forever; split sizes never change; show = remove class → exact geometry back. NEVER remove the leaf on hide, NEVER disconnect the component, NEVER reintroduce slot memory — the Session 25 machinery (slot modes, serialization of slots) is deleted and its machinery must not return.
- **Tab-activation hand-off**: when the hidden leaf is the group's active tab, the next VISIBLE leaf must take activation (`_handOffHiddenTab`); the joint "all leaves hidden" case must keep them all hidden (return early).
- **Probe-order rule**: when a probe both re-shows a restored panel AND shows another never-docked panel, re-show FIRST — the capacity re-split (Session 24 rules) is legitimate and will squeeze the restored panel's geometry.
- **Panel that never had a tree leaf** (font/kerning/glyphs shown for the first time): still goes through `_addPanelLeaf` → Session 24 capacity placement; document that showing font at 762px re-splits root to thirds by design.
### Session 27 (current)

**Objective**: User vetoed Session 26's CSS-placeholder hide: "the hiding logic is broken - you should move the component OUT of the docking system, otherwise hiding a panel still occupies space, the title is still there, it is not hidden at all". New contract: **hiding removes the leaf from the dock tree entirely (no space, no title)** while **the component stays alive in a body-level hidden host** (same-document `appendChild` move never fires `disconnectedCallback`, so bus/loop/state survive - the Session 25/26 invariants). Session 26's `.dock-hidden`-on-leaf machinery is DELETED.

**Changes** (all `js/ui/dock_layout.js` unless noted):
1. **Remove-style hide**: `hidePanel` = `_captureHiddenSlot` -> `_removeLeaf` (gone from tree + DOM) -> move `_componentRefs[id]` into `_ensureHiddenHost()#dock-hidden-host` (initially-null `_hiddenHost`; `display:none` inline, appended to BODY) -> `_hiddenPanels.add` -> `_rebuild()`. ORDER TRAP: the component MUST move to the host BEFORE `_rebuild()` (rebuild wipes `container.textContent="", killing any comp still inside).
2. **`showPanel`** = if slot memory exists and its parent is still in the tree (`_isNodeInTree`), splice the leaf into that parent at the remembered index + re-apply extent/sizing (root/sibling-root modes serialize; `split`/`tabs` modes are runtime-only); if no slot or the subtree died, fall back to `_addPanelLeaf` (Session 24 capacity contract). Then restore the component from the host via the normal `_buildDOM` -> `_attachComponentElements` re-attach path (it re-appends `contentAreas[0]` because the area exists again). `_restoreHiddenSlot` runs BEFORE deleting the memory entry.
3. **`_applyHiddenPanelPlacement()`** (L~1840) - called at the TOP of `_buildDOM` (L~451) and `deserialize` (L~336) BEFORE rebuild: for every panel in `_hiddenPanels` that still has a tree leaf (old Session 26 blobs, or leaves re-introduced by stale serialized state), `_removeLeaf` it + reparent its component into the hidden host. Belt-and-suspenders: normal hide flow already guarantees no leaf, this converges legacy/foreign states.
4. **Slot memory**: new `_captureHiddenSlot` / `_restoreHiddenSlot` / `_serializableHiddenSlots` / `_findPanelLeafSlot` / `_isNodeInTree`; `hiddenSlots` persisted via `_saveStateToStorage` and restored in `_restoreFromStorage`. Only `root` and `sibling-with-root` modes survive serialization; `split`/`tabs` modes are runtime-only (fall back to `_addPanelLeaf` next boot - same as Session 23/24 behavior).
5. **Float branch**: hiding a floated panel removes it from the float system entirely (window + entry destroyed after the component is safe in the host); `_restoreFromStorage` SKIPS re-creating float windows for panels in `_hiddenPanels`. Session 26's float `.dock-hidden` re-application line is gone.
6. `.dock-hidden` CSS remains ONLY as inactive-tab semantics (`_activateTab`/`_createTabsEl`); `_handOffHiddenTab`/`_applyHiddenStateToDOM` deleted.
7. **driver re-semanticized** (`test/dock_panels_driver.mjs`): B8/B19/C3/D3/D5/D13 assert leaf REMOVED + component hosted in `#dock-hidden-host` (`!!hosted`, `gone:true` = not in DOM tree) + `canvasAlive` (loop running); D13 back to "deserialize drops hidden-panel leaves". NEW gotchas: D10/D11 must `_hiddenPanels.delete('font')` before `_insertAtPanel` - a real drop source is always visible, and a hidden insert would be reverted by `_applyHiddenPanelPlacement` during `_rebuild`.

**Verification**:
- `test/dock_panels_driver.mjs` 44/44 ALL GREEN: B1-B19 (B8: font gone from tree + hosted + components alive), C1-C5 (C3: hidden survives reload, leaves dropped, comps hosted), D1-D14 (D3/D5: leaf removed + hosted + `wrapAttached`; D6 re-show reattaches + renderOk; D8 resize persistence sum=100; D9 narrow fresh layout trio hidden; D10/D11 insert-into-full/perpendicular after hidden-clear; D12 crowded restore folds overflow + persistence; D13 deserialize drops hidden leaves + hosted; D14 zero app errors).
- `test/canvas_hide_sync_probe.mjs` 7/7 PASS: S2 canvas hidden -> tree drops canvas leaf ([75,25] -> 4-leaf v-split), canvas alive in host (inDOM, loop running, ZERO disconnect); S3b/S3c dispatch + undo WHILE HIDDEN work (bus listeners permanent - Session 25 trackers split holds); S4 re-show restores rect [28,53,543,375] EXACTLY + tree [75,25]; S4b/S4c dispatch + undo after re-show; S5 post-reload canvas boots hidden IN HOST (rect 0,0,0,0, loop running) then re-show restores [543,375] + [75,25] tree; S6 real wheel + keydown work. FINAL errs [].
- LSP diagnostics clean (dock_layout.js only app-code change).
- First driver run showed D10/D11 failing: hidden font had no leaf so `_insertAtPanel` re-inserted it, then `_applyHiddenPanelPlacement` reverted it (assertion saw the pre-insert tree). Root cause = test calling `_insertAtPanel` on a hidden panel (impossible from real UI - drop source is a visible dock leaf). Fixed in driver by clearing `_hiddenPanels.delete('font')` before the simulated drop.

**Notes for future sessions**:
- **Session 27 hiding contract (SUPERSEDES Sessions 24/25/26)**: hide = leaf removed from tree (no space, no title) + component parked in `#dock-hidden-host` under BODY (DEAD if any code removes/disconnects it). The host container is re-created per `_ensureHiddenHost`; components become display:none children of BODY - DO NOT treat "in DOM" as "in dock". Show = slot splice or `_addPanelLeaf` + re-attach via normal `_buildDOM`. Never bring back `.dock-hidden`-on-leaf CSS hiding; never re-add `_applyHiddenStateToDOM`.
- **same-document moves are free**: `appendChild`/`insertBefore` within one document never fire `connectedCallback`/`disconnectedCallback`, so parking/unparking is safe and loop/bus/state persist. First paint of a moved comp re-runs layout work (S4 backing 543x375 exact restore).
- **Capacity contract intact**: `_addPanelLeaf`/`_addNodeToTree` Session 24 rules still govern first-show placement; showing font at 762px re-splits root to thirds by design (probe-order rule from Session 26 still applies).
- Canonical probe: `test/canvas_hide_sync_probe.mjs` (S1b/S3b/S3c/S4/S4b/S4c/S5/S6 + 7 hard assertions). Driver: `test/dock_panels_driver.mjs` 44 assertions.

### Session 28 (current)

**Objective**: `test/sample_panel_cdp_driver.mjs` suddenly failed with 25 errors (every ink=0, canvas height 235, wrap height 0) after a large-scale change session. Determine whether the app regressed or the driver environment broke.

**Verdict: NOT an app regression** — kern driver 19/19, shortcuts 33/33, dock driver 44/44 all green; the sample panel code was untouched since 27850f2. Root cause: **stale dock-layout localStorage pollution**. A previous driver/probe run left `inkshader_dock_layout_v2` with the sample panel inside an INACTIVE tab (leaf `display:none`), so every wrap/canvas measurement read 0. Fully reproduced: injecting a tabs blob (canvas active at index 0, sample at index 4) via a pollution probe made the unpatched driver fail with the EXACT same 25 errors; a healthy layout passed 72/72. Diagnostic driver confirmed the panel itself is fine (ink 757, canvas [235,92], minH 92px, facade direct-draw 1144).

**Changes** (driver-only, no app code):
1. **`test/sample_panel_cdp_driver.mjs` layout hygiene (Phase A start)** — replace `localStorage.removeItem('inkshader_dock_layout_v2')` with SEEDING a known layout + reload: tree = split h [75,25], children = canvas leaf + sample leaf, `hiddenPanels` must list ALL SIX ids (`['objects','properties','console','font','kerning','glyphs']`), floats [], hiddenSlots []; then `navigate(APP_URL + '&clean=1')` + waitReady. Lesson: with a RESTORED layout the app never seeds the optional trio hidden by itself, and any core panel missing from the tree gets re-added (capacity placement) — both would crowd the sample leaf.
2. **Phase B blob** — its old layout blob (no `hiddenPanels` field) now carries explicit `hiddenPanels: ['font','kerning','glyphs']` — `_restoreFromStorage` ADDs the trio to whatever hidden set exists (does not reset), so Phase A's seed would otherwise leave objects/properties/console hidden and 'legacy panels intact' would fail.
3. **Measurement fix for the last 2 failures — `clientHeight` → `offsetHeight`** (L158/L169 wrap-size check, L399/L407 min-height checks): at fontSize 96 the natural line (188+24 = 212px) exceeds the 25%-leaf wrap width (~172px), the canvas backing widens to 212px, a HORIZONTAL scrollbar appears and eats exactly 6px of `clientHeight` (255 → 249; squeezed state 116 → 110) without changing the layout size at all. `offsetHeight` (border-box) is scrollbar-independent: 257 both states, 118 squeezed — the checks measure what they claim ("wrap size", "min-height floor"). Verified by `sample_panel_diag2_driver.mjs` dumps (at48: sw 172 = cw 172 no h-bar; at96: sw 212 > cw 172 h-bar present; m96.maxRowWidthPx 188).

**Verification** (headless Chrome :9222, server :8123, CDP Network.clearBrowserCache, pollution probe run first) — **Phase A 66/66 + Phase B 6/6 = 72 checks ALL GREEN**:
- Previously failing checks now pass: `wrap size unchanged` `{"wrapH":257,"wrapNow":257}` (was 255→249); `min-height tracks font size (96 -> >= 116)` `{"minH":118}` (was 110).
- Unpatched driver under the same pollution: Phase A 25 errors / 68 checks — the failure family is 100% environmental (stale layout), not code.
- Full suite intact: same-row, kerning gap, content-driven height, min-width/min-height, scrollbars, persistence, backslash syntax, trailing/middle/lone newlines, multi-line guides, undo/redo, Phase B restore.
- Temp probes deleted after the investigation: `test/sample_panel_diag_driver.mjs`, `test/sample_panel_diag2_driver.mjs`, `test/pollute_dock_probe.mjs` (findings captured here; the driver supersedes them).

**Notes for future sessions**:
- **Stale-layout failure fingerprint**: "canvas renders glyphs: 0" + "canvas height: 235" (CSS default) + wrap 0×0 measurements while dock/kern/shortcuts drivers stay green = check `inkshader_dock_layout_v2` for a leaf with `display:none` ancestors (inactive tab). The app is fine; the environment is polluted.
- **Seed-layout contract**: the Phase A seed MUST include all six hidden ids — restored layouts never hide the optional trio themselves, and missing core panels get re-added into the tree and crowd the sample leaf. Phase B's blob must also declare `hiddenPanels` explicitly (restore ADDS, never resets).
- **Scrollbar-collapse physics**: a horizontal scrollbar (canvas backing wider than the wrap) steals 6px of `clientHeight` vertically — the wrap LAYOUT height is unchanged. Any check asserting "component size unchanged" or "min-height floor" must measure `offsetHeight` (border-box), not `clientHeight`. `clientHeight` stays correct for overflow checks (`scrollHeight > clientHeight`).
- **25% leaf width is the narrow case**: at fontSize 96 the natural 'AB' line is 212px; a 25%-of-762px leaf (~172px) triggers the h-scrollbar by design (Session 22 min-width contract). Do not widen the seed to dodge the measurement — offsetHeight is the correct fix.
- Driver check counts: Phase A 66 + Phase B 6 = 72 (unchanged count; two checks' measurements re-semanticized).

### Session 29 (current)

**Objective**: Keyboard-shortcut discoverability pass — ① File menu items show their global shortcut labels (reference the Edit menu convention); ② Help menu gains an entry that opens the help-modal (already existed); ③ help-modal shortcut list is the complete bilingual (en/zh) set; ④ toolbar buttons' `data-tip` append the shortcut hint.

**Audit outcome**: all four items were ALREADY implemented in code (verified line-by-line + real-browser probes); this session's work was the skeptical re-verification, one probe-side correction, and the regression probes.

**Verified state (no app-code changes needed)**:
1. **`js/ui/layout_shell.js`** — File menu `FILE_SHORTCUTS` (L172-178): `file.new_project` Ctrl+N / `file.load_json` Ctrl+O / `file.save_json` Ctrl+S / `file.save_ufo` Ctrl+Shift+E / `file.save_svg` Ctrl+Shift+S; `makeItem` attaches `shortcut` (L182); Help menu `help.shortcuts` entry opens `help-modal` (L403-408). `js/ui/dropdown_menu.js` L108-113 renders `item.shortcut` into a `.shortcut` span; CSS `.save-dropdown-item .shortcut` (style.css ~L2774) right-aligns it.
2. **`js/ui/help_modal.js` + `js/services/i18n.js`** — 20-entry shortcut list (L13-32) with `data-i18n="help.s.*"`; all 20 keys present in BOTH en (L50-51) and zh (L137-138). List matches `handleWindowKeydown` (canvas_input_controller.js L1934-2057) 1:1: tools V/A/P/O/M, C/S/Y + I/J/B (NODE-only), Ctrl+U boolean family (incl. Alt/Shift variants), Ctrl+Shift+X, Ctrl+N/O/S, Ctrl+Shift+E/S, Ctrl+Z/Y(+Shift+Z), Ctrl+C/V/D, Del, Space+drag, Ctrl+Arrow, Ctrl+Wheel/-/+, Esc.
3. **`index.html`** — every toolbar button carries `data-tip` with its shortcut: (V)(A)(P)(O)(M)(C)(S)(Y)(I)(J)(B)(Ctrl+U)(Ctrl+Shift+U)(Ctrl+Alt+U)(Ctrl+Alt+Shift+U)(Ctrl+Shift+X); rendered via `.tool_button:hover::after { content: attr(data-tip) }` (style.css ~L256-275). `data-i18n-tip` translations (tool.*/mode.*/action.*) already embed the same shortcut text (en L41-45 / zh L128-132) — `I18nManager.translateDOM` (i18n.js L207-209) syncs them into `data-tip`, so tooltips stay bilingual + shortcut-annotated.

**Verification** (headless Chrome :9222, server :8123, CDP Network.clearBrowserCache — ALL GREEN):
- `test/shortcuts_cdp_driver.mjs` **33/33**: tool switching V/A/P/O/M dispatched + applied; node modes C/S/Y + ops I/J/B (and I ignored outside NODE); boolean family all four variants + no double-dispatch; Escape clears selection + drops uncommitted draw path + resets IDLE; Space sets `_spaceDown` / space+drag PANNING / keyup clears / plain drag doesn't pan / Space ignored inside input; Ctrl+N/O/Shift+S(exportToSVG)/Shift+X; Ctrl+S wired.
- `test/menu_shortcut_probe.mjs` **16/16** (new, kept): File menu renders Ctrl+N/O/S + Ctrl+Shift+E/S labels (import items correctly have none); Edit menu Ctrl+C/V/D + Del intact (regression); Help menu entry opens help-modal; help list ≥ 20 entries; zh `help.s.*` keys resolve; modal closes via OK button.
- **Probe-side bug found (not an app bug)**: first probe run guessed wrong menu labels ("Open Project"/"Quick Save"/"Export UFO"/"Export SVG") — actual labels are `file.load_json` → "Load Project (JSON)", `file.save_json` → "Save as JSON Project", `file.save_ufo` → "Save as UFO Project", `file.save_svg` → "Save as SVG File". All four failures were probe-label mismatches; corrected in the probe, re-run 16/16.
- LSP diagnostics clean: layout_shell.js, help_modal.js, i18n.js.

**Notes for future sessions**:
- **Menu-shortcut convention**: File/Edit menu items pass `shortcut` to `makeItem`; dropdown_menu.js renders `.shortcut` automatically. Only items with a REAL global binding (canvas_input_controller.js `handleWindowKeydown`) get a label — load_ufo/load_svg intentionally have none.
- **Shortcut truth source**: the canonical binding list is `handleWindowKeydown` (canvas_input_controller.js L1934-2057). Any UI shortcut hint (menus, help modal, tooltips) MUST mirror it; the header comment (L1931) and the help modal are the two sync points.
- **Tooltip chain**: `data-i18n-tip` → translateDOM → `data-tip` → CSS `::after`. Toolbar translations (tool.*/mode.*/action.*) embed shortcut text in BOTH locales — keep them in sync when a binding changes.
- Canonical probes: `test/shortcuts_cdp_driver.mjs` (33 checks — key handling) + `test/menu_shortcut_probe.mjs` (16 checks — label rendering + help-modal open path). Rerun both for any menu/help/tooltip/shortcut change.

### Session 30 follow-up — glyph grid leftover space (user: "宽度800是8列、890还是8列、900才9列，但800到890多出来的宽度应该放在组件最右边，而不是平摊到每一列；列间距出于美观不应该有任何改变")

**Root cause**: the dock grids used `repeat(cols, 1fr)` — between two column thresholds the extra width was distributed INTO every column (columns stretched past the 68px target while the 4px gap stayed fixed), which is exactly the "spacing gets stretched" look the user rejected.

**Changes** (`js/ui/glyph_popup.js` only):
1. `_refreshCharGrid` + `_renderNoCodeGroups` — `gridTemplateColumns` = `repeat(${cols}, ${GlyphPopup.COLUMN_W}px)` (fixed 68px columns). Leftover width now sits at the RIGHT of the grid automatically (grid default `justify-content: start`); columns and the 4px `.seq-menu-grid` gap are constant at every width.
2. `_computeCols` — now accounts for the real grid gap: N columns fit iff `N*COLUMN_W + (N-1)*GRID_GAP <= w`, i.e. `floor((w + GRID_GAP) / (COLUMN_W + GRID_GAP))`. New constants: `GRID_GAP = 4` (matches `.seq-menu-grid` CSS gap) and `SECTION_PAD = 16` (the `.seq-menu-section` horizontal padding `8px + 8px`; old `MENU_PAD = 12` was a guess — the empirical samples couldn't discriminate 12 vs 16). Width source: the char grid's OWN `clientWidth` when it exists (self-corrects against future CSS padding changes), falling back to `menu.clientWidth − SECTION_PAD` before the grid is built on the first `_buildMenu` call.

**Verification** (`test/glyph_cols_title_probe.mjs` extended to **23 checks**, ALL GREEN; dock driver 44/44 re-run green; LSP clean; 0 app errors):
- New geometry-stability check (the user requirement): leaf forced to 340px vs 360px — both yield 4 columns — item width **68 == 68**, inter-item gap **4 == 4**, last column right edge **284 == 284** (identical), while grid width grew 324 → 344 (the +20px is leftover on the right, not in columns). Formula match re-verified at every width: 138px grid → 1 col, 231px → 3, 618px → 8.
- All prior checks intact (titles removed, seq-bar menu fixed 556px + `repeat(8, 1fr)` untouched).

**Notes for future sessions**:
- **Glyph grid geometry contract**: column width 68px + gap 4px are CONSTANT across all widths; the column count is the only thing that changes (`floor((gridW+4)/72)`), and leftover space accumulates right. Do NOT revert to `1fr` columns — that stretches columns/gaps between thresholds (the rejected behavior). The sequence-bar add menu keeps `1fr` only because it SELF-SIZES to `cols×68+12`, so 1fr == 68px there anyway.
- If `.seq-menu-section` padding or `.seq-menu-grid` gap ever changes in CSS, update `SECTION_PAD`/`GRID_GAP` in glyph_popup.js (or rely on the grid-width measurement for pad and just keep GRID_GAP in sync).

### Session 30 (current)

**Objective**: User request: ① remove the "Add Kerning" title from the kerning component; ② remove the "Add Glyph" title from the glyphs component (note: the glyph picker is activated in MANY positions — both the dock `glyph-popup` panel AND the sequence-bar add/insert menus share the same UI); ③ the glyphs component was originally fixed-width with 8 grid columns, but as a dock panel its width is now adjustable — make the column count width-dynamic; the sequence-bar add-button scenario keeps its FIXED width/columns (do not change).

**Changes**:
1. **`js/ui/kern_popup.js`** — Removed the `<div class="seq-menu-header">` block (the "Add Kerning" `.seq-menu-title` span was the header's only content). The add-row/list markup is untouched.
2. **`js/ui/glyph_popup.js`** — Removed the "Add Glyph" `.seq-menu-header` block. Replaced `static get COLS() { return 8; }` with width-driven columns: `static get COLUMN_W() { return 68; }` / `static get MENU_PAD() { return 12; }` (same consts as the sequence bar menu) + `_computeCols()` = `max(1, floor((this._menu.clientWidth − MENU_PAD) / COLUMN_W))`. `_buildMenu` stores `this._cols` (no closure constant anymore; `refreshSections` and the initial renders read `this._cols`). A `ResizeObserver` on the `.sequence-add-menu` recomputes `_cols` and re-renders the grids only when the column count actually changes. Top-of-file comment updated ("header with title" removed).
3. **`js/ui/glyph_sequence_bar.js`** — Removed the `title` span from the `_addMenu` header ONLY (the "Add Glyph" `.seq-menu-title`); the `.seq-menu-close-btn` close button is kept (the floating menu's only explicit dismiss control). Fixed-width/cols logic (L571-583: 8 cols default, `cols = max(4, floor((availW−12)/68))` on narrow screens, `menuW = cols*68+12`) UNTOUCHED.
4. **`js/services/i18n.js`** — Removed the `kern.title` keys (en L87 + zh L173) — dead after the kern_popup edit (data-i18n was its only consumer).

**Verification** (pure-Node CDP driver `test/glyph_cols_title_probe.mjs`, headless Chrome :9222, server :8123, CDP Network.clearBrowserCache, baseline `test/InkShader_project_2026-08-02T16-27-25.json` — **22/22 ALL GREEN**, zero app errors):
- Kern panel shown via `dock.showPanel('kerning')`: no "Add Kerning" text anywhere in the document, no `.seq-menu-header` inside the component, add-row + list intact, pair rows render (A→E −500 after baseline load).
- Glyph panel shown: no "Add Glyph" text anywhere, no `.seq-menu-header`/`.seq-menu-title`, 95 char cells. `_cols` matches the width formula exactly: initial leaf 247px → 3 cols (`repeat(3, 1fr)`); leaf forced to 160px → 2 cols; 640px → 9 cols; restore → 3 cols. (Probe forces BOTH `flex` and `width` on the leaf — the panel can land in a horizontal OR vertical split, where flex-basis alone controls the other axis.)
- Sequence-bar add menu (opened via `.seq-bar-ins-btn`): no "Add Glyph" title, NO `.seq-menu-title`, close button present and functional (menu removed on click), fixed sizing intact — `menu.style.width` exactly 556px (8×68+12) and char grid `repeat(8, 1fr)`.
- Regressions: `test/dock_panels_driver.mjs` **44/44**; `test/kern_history_cdp_driver.mjs` **19/19**. LSP diagnostics clean: kern_popup.js, glyph_popup.js, glyph_sequence_bar.js, i18n.js.

**Notes for future sessions**:
- **Glyph-picker title rule**: the "Add Glyph" header existed in BOTH the dock glyph panel and the sequence-bar add menu (same UI family, opened from many positions: `.seq-bar-add-btn`, `.seq-bar-ins-btn` per token, `.seq-bar-ins-last-btn`). Any future header/title change to this UI must hit both sites; the sequence-bar one additionally carries the close button — don't remove the whole header there or the floating menu loses its explicit X (outside-click still closes it).
- **Two width regimes**: dock `glyph-popup` = width-adaptive columns (`_computeCols`, ResizeObserver-driven re-render, ~68px/column target, min 1); sequence-bar add menu = SELF-SIZED fixed width (menuW = cols×68+12, 8 default, floor-shrink only when the window edge forces it) — its width and column logic must never be coupled to the dock panel's.
- Canonical probe: `test/glyph_cols_title_probe.mjs` (22 checks — rerun for any kern/glyph/seq-bar title or glyph-grid column change). test/ inventory now 51 files (36 probes/drivers + 15 fixtures/inputs).

### Session 31 (current)

**Objective**: User request — dock resize min-width instability, two symptoms: (1) after dragging a panel to its minimum width, the drag stops entirely instead of continuing to compress the opposite side ("dead seams"); (2) multi-tab merged panels sometimes end up BELOW the 200px minimum. User constraint: all components must work through the dock's unified interface — no per-panel exceptions ("所有组件都应该在docking系统中使用统一接口，不具有任何特殊性").

**Root cause** (`js/ui/dock_layout.js`): `_onResizeMove` clamped every child of a split against ONE flat `minPct = max(5, minPixel/total×100)`, treating a nested subtree as an atomic 200px unit. A same-direction nested h-split (e.g. root h `[canvas | h-split[tabs, console]]`) has a TRUE minimum of children-mins + gaps (200+200+6 = 406px), but the flat clamp crushed it to 200px → inner tabs/console rendered at ~149px each (symptom 2). Once every inner child sits below its LOCAL min, `capacity = max(0, startPct − minPct)` sums to 0 → `clampedShrink ≤ 0` → the seam is dead in both directions (symptom 1). The header comment "the resize clamp refuses to move rather than corrupt sizes" documented this deliberate refusal.

**Changes** (`js/ui/dock_layout.js` only):
1. New recursive `_nodeMinPx(node, dir)`: leaf/tabs = flat min (200 width / 100 height); split with same direction = sum of children mins + `RESIZER_GAP × (n−1)`; split with perpendicular direction = max of children mins. Commented with "Session 31".
2. `_onResizeMove` — replaced the flat `minPct` with per-child `minPctOf(i) = max(5, (childMins[i]/r.total)×100)`; `childMins` comes from `r.splitEl._treeNode.children.map(c => this._nodeMinPx(c, r.direction))`, falling back to flat mins when the tree node is missing/mismatched (guard: children count must equal `r.children.length`). ⚠️ gotcha: inside that function the child array variable is `r.children`, NOT a bare `children` — first attempt used `children` and threw a ReferenceError that silently killed ALL resize drags (probe showed 4 frozen-width FAILs); fixed to `r.children` → all green.
3. `_fitsNewSlot` — replaced the flat `_maxSlots` check with a recursive capacity check: `extent >= childrenMin + flatMin + RESIZER_GAP × children.length`, where childrenMin sums `_nodeMinPx` over existing children — stops drops that would create a nested state below the sum-of-mins. `_maxSlots` is NOT dead: still used at L164/L319/L1736.

**Verification** — new probe `test/dock_min_probe.mjs` (9 checks), pure-Node CDP (Chrome :9222, server :8123), direct tree-mutation seeding (`window.__dockH.setTree` → `d.root = tree; d._buildDOM()`):
- **PRE-FIX run: 7/9, deterministic reproduction of BOTH symptoms** — R1b FAIL: root seam +400px crush left properties/console/tabs at **149.4px each** (below the 200 min); R1c FAIL: inner-seam drags of +150 and −150 moved NOTHING (before/after snapshots byte-identical — dead seam). All controls passed (R2 flat cascade, R3 growth-into-subtree, R4 flat tabs group).
- **POST-FIX run: 9/9 ALL GREEN** — R1b: crush stops at the subtree's true min (~29% ≈ 406px), tabs/console = 202.4/202.3px (pre-fix: 149.4); R1c: inner seam redistributes within the legal slack (205.2↔199.5), never below min, no dead-lock; R1d: recovery drag restores the root (sizes 35.3/64.7); R2/R3/R4 numbers byte-identical to pre-fix (379.5/199.7/818.8 …) — flat layouts untouched.
- **Regressions (run SEQUENTIALLY — see lesson below)**: `dock_panels_driver.mjs` **44/44**, `kern_history_cdp_driver.mjs` **19/19**, `glyph_cols_title_probe.mjs` **23/23**, `canvas_hide_sync_probe.mjs` **7/7**. LSP diagnostics clean on dock_layout.js.

**Notes for future sessions**:
- **Seeding lesson**: `deserialize` runs `_normalizeOverflowingSplits` which folds over-capacity nested splits into tabs — a violating tree state can NEVER be reached via deserialize (it gets normalized away), yet runtime drags leave the tree exactly in that un-normalized state. Probes asserting violation behavior MUST seed via direct tree mutation (`d.root = tree; d._buildDOM()`), never via stored-layout restore.
- **Geometry lesson**: min-width constraints only accumulate in SAME-DIRECTION nesting (h-split inside h-split); a v-split child inherits its parent's width, so its width-min does not participate. Asserting merged tabs groups requires `t.n > 1` filtering (the canvas wrapper is also a `.dock-tabs`).
- **NEVER run multiple CDP drivers in parallel on one headless Chrome**: first batch (4 concurrent) failed spuriously — glyph probe timed out at 180s, kern_history 4 undo/redo null-results (first-undo-OK/later-dead = `is_restoring` rAF timing race under load), dock driver B13/B14 "kern selects empty" — all vanished when re-run standalone. Shared origin localStorage + single-thread rAF make concurrent drivers mutually polluting. Sequential multi-run = the protocol.
- **Nested-min contract (user-facing)**: a same-direction nested split's floor is the SUM of its children's floors + gaps, not the flat 200px — the resize clamp now refuses only at that true floor. Perpendicular nesting floors are the max of children floors (child inherits parent extent on that axis).
- Canonical probe: `test/dock_min_probe.mjs` (9 checks — rerun for any resize-clamp or drop-capacity change in dock_layout.js).
